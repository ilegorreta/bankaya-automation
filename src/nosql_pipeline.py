...
def main():
    """Read local JSON files and upload the data into DynamoDB tables"""
    nosql_files = ["customer.json", "item.json", "orders.json"]
    # Iterate through the each local file
    for file in nosql_files:
        # Read JSON file as Python Dict
        ...
        if file == "customer.json":
            ...
            table = ORDERS_TABLE
        # Insert dictionary records into DynamoDB
